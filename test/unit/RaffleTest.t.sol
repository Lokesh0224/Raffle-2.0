...
contract RaffleTest is Test {
    ...
    function testFullfillrandomWordsPicksAWinnerResetsAndSendsMoney() public raffleEntered{
        ...
        //This loop is for the additional players
        for(uint256 i = startingIndex; i < startingIndex + additionalEntrants; i++){
            address newPlayer = address(uint160(i)); //converting number to address
            hoax(newPlayer, 1 ether);
            raffle.enterRaffle{value: entranceFee}();
        }
        uint256 startingTimeStamp = raffle.getLastTimeStamp();//constructor timestamp
        ...
        uint256 endingTimeStamp = raffle.getLastTimeStamp();//timestamp inside the fulfillRandomWords
